...
Webservice  -> Verifier   : start pxh-auth : invitation
...
note left of Verifier : session information is conveyed via ""session"" parameter\n(which contains the invitation_msg_id).

HolderAgent  -> Verifier : POST https://webservice.com/pxh(body: <JWT_VP>)
