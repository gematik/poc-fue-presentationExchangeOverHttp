...
Webservice  -> Verifier   : start pxh-auth(presentation definition, callback) : invitation
...
HolderAgent <-- Verifier    : 200 OK(body: <Auth Request>)
...
note left of Verifier : session information is conveyed via ""session"" query parameter.

HolderAgent  -> Verifier : POST https://webservice.com/pxh?session=<invitation_msg_id>(body: <JWT_VP>)
...
Verifier ->> Webservice : verification result
