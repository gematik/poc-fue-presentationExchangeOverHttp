...
- usable with mobile wallets (not only), 'always on' connectivity assumed, no persistent connection needed (session based approach)
...
participant "Client\nApplication" as UserAgent
participant "Web Service\n+ Verifier" as webservice
...
webservice  -> webservice : create invitation
...
note left of webservice : session information is conveyed via ""session"" parameter.

HolderAgent   ->  webservice : POST https://webservice.com/pxh(body: <JWT_VP>)
HolderAgent   <-- webservice : 200 OK

== Web Service verifies authentication ==

webservice  -> webservice : check JWT\nsignature
webservice  -> webservice : verify credentials

alt verification OK
    UserAgent     <-- webservice : grant access
else verificationf failed
    UserAgent     <-- webservice : deny access
end
